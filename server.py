...
def loadCategoryItems(catalog_id):  # show items for a specific category
    ...
    items = session.query(
        ...
        category_id=catalog_id).order_by(desc(catalogItem.id))
    if 'username' not in login_session:
        state = ''.join(
            random.choice(string.ascii_uppercase + string.digits)
            for x in xrange(32))
        ...
        return render_template(
            ...
            categories=categories,
            items=items,
            rightmsg="Category " + category.name +
            " items showing the latest first",
            getCategoryName=getCategoryName)
    else:
        return render_template(
            'allobjectsPrivate.html',
...
def editItem(item_id):  # edit an item
    if 'username' not in login_session:
        response = make_response(
         json.dumps(
            'Security breach!!! You are trying to' +
            ' edit item without login!!!'), 401)
    ...
    if login_session['user_id'] != item.user_id:
        response = make_response(
         json.dumps(
            'Security breach!!! You are trying to' +
            'edit item which was not created by you!!!'), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
    if request.method == 'GET':
        categories = session.query(Category)
...
def deleteItem(item_id):  # delete an item
    if 'username' not in login_session:
        response = make_response(
         json.dumps(
            'Security breach!!!' +
            'You are trying to delete item without login!!!'), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
    item = session.query(catalogItem).filter_by(id=item_id).one()
    if login_session['user_id'] != item.user_id:
        response = make_response(
         json.dumps(
            'Security breach!!!' +
            'You are trying to delete item which ' +
            'was not createdby you!!!'), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
    item = session.query(catalogItem).filter_by(id=item_id).one()
    if request.method == 'GET':
...
def gconnect():  # log in through google api
    ...
    if result['user_id'] != gplus_id:
        response = make_response(json.dumps('''Token's user ID dosen't
    ...
    if stored_access_token is not None and gplus_id == stored_gplus_id:
        response = make_response(json.dumps('''<h5>Current user is
             already connected</h5>'''), 200)
        response.headers['Content-Type'] = 'application/json'
        return response
...
def fbconnect():  # log in through facebook api
    ...
    url = url + 'fb_exchange_token&client_id'
    url = url + '=%s&client_secret=%s&fb_exchange_token=%s' % (
    ...
    from the server token exchange
    we have to split the token first
    on commas and select the first
    index which gives us the key :
    value for the server access token
    then we split it on colons to pull
    out the actual token value
    and replace the remaining quotes
     with nothing so that it can be
    used directly in the graph api calls
    '''
    token = result.split(',')[0].split(':')[1].replace('"', '')
    url = 'https://graph.facebook.com/v2.8/me?access_token'
    url = url + '=%s&fields=name,id,email' % token
    h = httplib2.Http()
    result = h.request(url, 'GET')[1]
    ...
    url = url + 'access_token=%s&redirect=0&height=200&width=200' % token
    h = httplib2.Http()
    result = h.request(url, 'GET')[1]
    data = json.loads(result)
...
def newItem():
    if 'username' not in login_session:
        response = make_response(
         json.dumps(
            'Security breach!!! You are trying to' +
            ' add new item without login!!!'), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
